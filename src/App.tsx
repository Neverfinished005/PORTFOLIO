...
const PortfolioScene = ({ mass, spin, activeIndex, scrollRef }: { mass: number, spin: number, activeIndex: number, scrollRef: React.MutableRefObject<number> }) => {
  useFrame((state) => {
    ...
    // The destination when scrolled fully into the black hole (scroll = 1)
    const bhTargetPos = new THREE.Vector3(bhX, bhY, bhZ);
    // The starting position (now much closer to the black hole)
    const galaxyTargetPos = new THREE.Vector3(0, 15, 60);

    const scroll = scrollRef.current;

    // Smooth transition curve for the drop
    ...
    // Look from Galaxy (y=200) down to Black Hole (y=0)
    // Always look at the Black Hole
  ...
  return (
    <>
      <BlackHole3D mass={mass} spin={spin} zoom={0} scrollRef={scrollRef} />
...
const PerfHUD = () => {
  ...
  return (
    <div className="fixed top-12 left-10 z-[60] font-mono text-[8px] uppercase tracking-[0.5em] opacity-40 flex flex-col gap-2 pointer-events-none text-white">
      <div className="flex items-center gap-4">
        <div className="w-1 h-1 bg-lensing" />
        <span>Core_Stable</span>
      </div>
      <div className="flex items-center gap-4 text-lensing">
...
export default function App() {
  ...
  const AVATAR_SRC = '/avatar.jpg'; // e.g. '/avatar.jpg'
  // ─────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    // We only call load if the source changed or on initial mount if needed, 
    // but React handles src changes. We'll let it be.
    if (isMusicPlaying) {
  ...
  useEffect(() => {
    ...
    // Attempt to play immediately (works in some browsers if previously interacted)
    if (audioRef.current) {
      ...
      <div className="h-[200vh]">
        ...
        <div className="fixed inset-0 z-0 bg-black pointer-events-none">
          <Canvas gl={{ antialias: false, powerPreference: "high-performance" }}>
            ...
            <PortfolioScene mass={mass} spin={spin} activeIndex={activeIndex} scrollRef={scrollRef} />
            <EffectComposer>
              <Bloom
                intensity={1.2}
        ...
        <div className="fixed inset-0 z-10 pointer-events-none flex items-end justify-center pb-36">
          <motion.div
            ...
            animate={{ opacity: hasScrolledDown ? 1 : 0, y: hasScrolledDown ? 0 : 30 }}
        ...
        <div className="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-5 pr-0
          md:gap-7">
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  onMouseEnter={() => {}}
                  onMouseLeave={() => {}}
                  onClick={() => setHoveredCard(isOpen ? null : i)}
                >
                  {/* Inner row: icon always visible, content visible when open */}
                  <div className="flex items-stretch" style={{ minWidth: '32px' }}>
                    ...
                    {/* Expanded content — fades in, wraps freely */}
                    <div
        ...
        {/* ── MUSIC PLAYER (top-center, hidden on very small screens) ── */}
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: hasScrolledDown ? 1 : 0, y: hasScrolledDown ? 0 : -20 }}
        ...
        >
          <button type="button" onClick={handlePrevTrack}
            className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-white/10 text-white transition flex-shrink-0">
          ...
          <button type="button" onClick={() => setIsMusicPlaying(v => !v)}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-white text-black hover:scale-105 transition transform flex-shrink-0">
            {isMusicPlaying
        ...
        <motion.div
          ...
          animate={{ opacity: hasScrolledDown ? 1 : 0, y: hasScrolledDown ? 0 : 40 }}
          transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: 0.5 }}
          className="fixed z-50 pointer-events-auto bottom-4 left-4 sm:bottom-6 sm:left-6 sm:w-72"
        >
          {/* ── MOBILE: small avatar circle (tap to open full card) ── */}
          <button
            className="sm:hidden w-14 h-14 rounded-full border-2 border-white/20 overflow-hidden shadow-xl backdrop-blur-xl bg-black/60 flex items-center justify-center"
          ...
          {/* ── DESKTOP: always-visible full card ── */}
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="flex items-center gap-2 pt-2.5 border-t border-white/10">
                <a href="https://github.com/Neverfinished005" target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded border border-white/10 text-white/40 hover:text-white hover:border-white/30 transition-all duration-200">
                ...
                <a href="https://www.linkedin.com/in/rudra-vable/" target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded border border-white/10 text-white/40 hover:text-[#0a84ff] hover:border-[#0a84ff]/40 transition-all duration-200">
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>
                  <span className="font-mono text-[8px]">LI</span>
                </a>
                <a href="https://www.instagram.com/rudr_a.25" target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded border border-white/10 text-white/40 hover:text-[#e1306c] hover:border-[#e1306c]/40 transition-all duration-200">
        ...
        {/* ── MOBILE PROFILE MODAL (slide-up full card) ── */}
        {profileOpen && (
          <motion.div
          ...
          >
            ...
            >
              ...
              <button onClick={() => setProfileOpen(false)}
                className="absolute top-4 right-4 w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-white/40"
              >
